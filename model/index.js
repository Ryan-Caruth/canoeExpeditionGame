...
function chooseACanoe(chooseCanoe) {
  if (chooseCanoe === "1") {
    ...
    Go to this link to learn how you guys navigate this lake: curl http://localhost:5000/onTheWater?location={Middle or Side}`;
...
function navigateTheLake(location) {
  if (location === "Middle" || location ==="middle") {
    ...
    curl http://localhost:5000/onTheRiver?direction={Enter left, right or middle}`;
    ...
    Which way do you go ? curl http://localhost:5000/onTheRiver?direction={Enter left, right or middle}`;
  } else {
...
function riverDirectionChoice(direction) {
  if (direction === "left" || direction ==="Left") {
    ...
    Please go here to see what happens next: curl http://localhost:5000/collisionOnRock`;
  } else if (direction === "right" || direction ==="Right") {
    return `I'm sorry ${giveName.name} and ${randomName}, a terrible decision is leading you over a waterfall. 
    This is a fatal fall. Would you like to play again? curl http://localhost:5000/startGame`;
    ...
    curl http://localhost:5000/hazard`;
  } else {
...
function inWaterDecision(option, paddle) {
  if (option === "grab" || option === "Grab") {
    if (paddle === "shore" || paddle === "Shore") {
      return `You guys were able to get back in the canoe and paddle to shore. Please click here to find out what your decision is: 
      curl http://localhost:5000/canoeToShore?shoreChoice={go or stay}`;
    } else if (paddle === "continue" || paddle === "Continue") {
      return `You guys are troopers for carrying on soaked. Please click here to determine what's next: 
      curl http://localhost:5000/continueSoaked`;
    } else
    ...
    if (paddle === "abandoned" || paddle === "Abandoned") {
      ...
      Please click this link to see what happens next: curl http://localhost:5000/choiceOnShore?survive={find or shore}.`;
    } else {
      return `${choice}, please enter {option=grab or land, paddle=shore, continue or {option=land, paddle = abandoned}.`;
  }
  } else {
...
function canoeToShore(shoreChoice) {
  if (shoreChoice === "stay" || shoreChoice === "Stay") {
    ...
    curl http://localhost:5000/waitOnShore`;
  } else if (shoreChoice === "Go" || shoreChoice === "go") {
    return `You guys decided to push on despite being cold and wet. Please click here to find out what happens next: 
    curl http://localhost:5000/continueSoaked`;
  } else {
...
function doTheyCommunicate(talking) {
  if (talking === "yes" || talking === "Yes") {
    ...
    curl http://localhost:5000/madeItToSite`;
    ...
    or swept away Please go here to see what happens next: curl http://localhost:5000/capsize`;
  } else {
...
function thunderAndLightning(choice) {
  ...
  if (choice === "go" && lightning === 'true' || choice === "Go" && lightning === 'true') {
    ...
    return `You are Lucky! You too have managed to escape the storm injury free. Please click this link to see what happens next: 
      curl http://localhost:5000/hooray`
      ...
      Please click this link to see what happens next: curl http://localhost:5000/hooray `
  } else {
...
function trueOrFalse(knowledge) {
  if (knowledge === "true" || knowledge === "true") {
    ...
    Would you like to play again? curl http://localhost:5000/startGame`;
    ...
    You win! Would you like to play again? curl http://localhost:5000/startGame`;
  } else {
